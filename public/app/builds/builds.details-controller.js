...
angular.module('OpenTMIControllers')
  .controller('BuildsDetailsController',
    ...
    var gotCb  = function(build) {
       $scope.build = build;
       $scope.buildStr = JSON.stringify(build, null, 2);
       $log.debug(build);
    };
    Builds.get({_id: $stateParams.buildId}, gotCb);
    var elfUrl = `/api/v0/duts/builds/${$stateParams.buildId}/elf/analyse`;
    ...
    d3.json(elfUrl, function(error, json) {
