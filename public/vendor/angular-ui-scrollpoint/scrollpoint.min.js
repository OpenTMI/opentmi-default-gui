...
(function() {
     ...
     */
    angular.module('ui.scrollpoint', []).directive('uiScrollpoint', ['$window', '$timeout', function($window,
        $timeout) {
        ...
        return {
            ...
            controller: function() {
                ...
                }; // ui-scrollpoint on top edge of element with top edge of target
                ...
                this.getEdge = function(scroll_edge, element_edge) {
                    if (scroll_edge && element_edge) {
                        if (this.edges[scroll_edge] && this.edges[scroll_edge][element_edge] &&
                            this.edges[scroll_edge][element_edge] !== true) {
                        ...
                        for (scroll_edge in absEdgeHits) {
                            for (element_edge in absEdgeHits[scroll_edge]) {
                                if (absEdges > 1 && absEdgeHits[scroll_edge][element_edge] < 0) {
                                    allPass = false;
                                } else if (angular.isUndefined(offset) || absEdgeHits[
                                        scroll_edge][element_edge] > offset) {
                    ...
                    if (fireActions) {
                        ...
                        if (self.actions) {
                            for (var i = 0; i < self.actions.length; i++) {
                                self.actions[i](edgeHit, self.$element, (self.hitEdge ? self.hitEdge
                                    .scroll : undefined), (self.hitEdge ? self.hitEdge.element :
                                    undefined));
                            }
                        }
                    }
            ...
            link: function(scope, elm, attrs, Ctrl) {
                ...
                attrs.$observe('uiScrollpointAbsolute', function(scrollpointAbsolute) {
                    ...
                    if (scrollpointAbsolute != absoluteParent) {
                        ...
                        uiScrollpoint.setTarget((!scrollpointAbsolute &&
                                uiScrollpointTarget) ? uiScrollpointTarget.$element :
                            null);
                        resetTarget();
                        uiScrollpoint.reset();
                    }
