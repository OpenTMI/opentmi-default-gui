...
function AngularD3multiParentDirective() {
    ...
    function link(scope, element, attr) {
        ...
        svg = d3.select(element[0]).append("svg:svg").attr("width", graph.width).attr("height", graph.height).append("svg:g").attr("transform", "translate(-" + graph.width * 0.2 + ", 0)");
        ...
        tree = d3.layout.tree().size([graph.height, graph.width / 2]);
        ...
        function update(){
            ...
            if( !scope.data || !scope.data.children )return;
            // calculate nodes
